...
import { Shield, Award, CheckCircle2, Copy, ExternalLink, Calendar, Fingerprint, QrCode, ArrowLeft, Sparkles } from 'lucide-react'

// Simulated certificate database - in production this would query Supabase
const MOCK_CERTIFICATES: Record<string, { id: string; name: string; module: string; issueDate: string; level: string; xp: number; hash: string; institution: string }> = {
  'CERT-2024-001': {
    id: 'CERT-2024-001', name: 'Carlos Mendoza', module: 'Frontend Fundamentals',
    issueDate: '2024-03-15', level: 'Colegio', xp: 2500, hash: '0x7A3F91BC4D2E08...',
    institution: 'Talento Tech Colombia'
  },
  'CERT-2024-002': {
    id: 'CERT-2024-002', name: 'Sofia Torres', module: 'React & State Management',
    issueDate: '2024-03-20', level: 'Universidad', xp: 5000, hash: '0x9B1F42DC7E3A05...',
    institution: 'Talento Tech Colombia'
  },
  'CERT-2024-003': {
    id: 'CERT-2024-003', name: 'Valentina Ruiz', module: 'HTML & CSS Superhero',
    issueDate: '2024-03-25', level: 'Escuela', xp: 1200, hash: '0x4C8D67FA1B2E93...',
    institution: 'Talento Tech Colombia'
  },
}
...
export default function CertificadoPage() {
  ...
  const [cert, setCert] = useState<typeof MOCK_CERTIFICATES[string] | null>(null)
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(`https://talento-tech.vercel.app/certificados/${params.id}`)
    setCopied(true)
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8 lg:p-12 relative z-10">
        ...
        {verifying ? (
          <div className="text-center py-32 animate-in fade-in">
            <div className="w-24 h-24 bg-purple-500/10 border border-purple-500/30 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-[0_0_50px_rgba(168,85,247,0.15)] animate-pulse">
              <Fingerprint className="w-12 h-12 text-purple-500" />
            </div>
            <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-4">VERIFICANDO_CERTIFICADO_</h2>
          ...
          <div className="text-center py-32">
            <div className="w-24 h-24 bg-rose-500/10 border border-rose-500/30 rounded-3xl flex items-center justify-center mx-auto mb-8">
            ...
            <div className="bg-[#0b1120]/80 border border-white/10 rounded-[3rem] p-8 md:p-14 backdrop-blur-xl shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-8">
                <div className="text-center">
                  ...
                  <p className="text-4xl font-black text-white italic tracking-tighter">{cert.name}</p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-8 border-t border-white/5">
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-sm font-black text-white">{new Date(cert.issueDate).toLocaleDateString('es-CO', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-sm font-black text-white">{cert.level}</p>
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-sm font-black text-amber-400">{cert.xp.toLocaleString()} XP</p>
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-[10px] font-mono font-black text-cyan-400 truncate">{cert.hash}</p>
                ...
                <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] text-slate-500 font-mono uppercase tracking-widest">ID: {cert.id}</span>
                  </div>
                  <div className="flex gap-3">
                    <button onClick={handleCopy} className="flex items-center gap-2 bg-white/5 border border-white/10 hover:bg-white/10 px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
