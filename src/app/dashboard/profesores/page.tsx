...
export default function ProfesoresDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 font-mono selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      <nav className="bg-[#0f172a]/95 backdrop-blur-xl border-b border-white/5 px-2 sm:px-4 md:px-8 py-3 flex justify-between items-center sticky top-0 z-50 shadow-2xl w-full">
        <div className="flex items-center gap-2 md:gap-10">
          <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setActiveTab('overview')}>
            <div className="bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border border-cyan-500/50 p-2 rounded-xl text-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.2)] group-hover:scale-110 transition-transform shrink-0">
              <Activity className="w-4 h-4 md:w-6 md:h-6" />
            </div>
            <div className="flex flex-col">
              <span className="font-black text-[15px] sm:text-xl md:text-2xl tracking-tighter text-white uppercase italic leading-none">
                COMMAND<span className="text-cyan-500 hidden sm:inline">CENTER</span>_
              </span>
              <span className="hidden sm:block text-[8px] font-bold text-cyan-500/60 tracking-[0.3em] uppercase mt-1">Ecosystem_Manager_V2</span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button 
            ...
            disabled={loading}
            className="flex items-center justify-center gap-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/20 px-3 md:px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all w-10 sm:w-auto h-10 sm:h-auto shrink-0">
